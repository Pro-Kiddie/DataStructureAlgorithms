...
// Each babyName int consists of only UPPERCASE alphabet characters of no more than 30 characters
...
// Tell Kattis how many baby names start with a prefix that is inside a given query interval [start…end), where start<end, and both are ints
...
class intBST_WithRank{
  ...
  BSTVertex* deleteNode(BSTVertex* root, int& key) {  
...
intBST_WithRank t; // change the data structure name
int MAXITR = 100;
...
//     int cmd;
//     intBST_WithRank male;
//     intBST_WithRank female;
//     while(cin >> cmd, cmd != 0){
//         switch(cmd){
//             case 1:{
//                 int name; cin >> name;
