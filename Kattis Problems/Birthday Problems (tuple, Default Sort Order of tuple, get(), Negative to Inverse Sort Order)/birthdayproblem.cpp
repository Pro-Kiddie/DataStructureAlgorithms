...
using namespace std; // we will utilize the natural sort order of tuple
...
int main() {
	...
	iii C = make_tuple(11, 13, -1983); // and then use NEGATIVE YYYY -> Ascending Age; Bigger year infront

	vector<iii> birthdays;
	birthdays.push_back(A); birthdays.push_back(B); birthdays.push_back(C);
	sort(birthdays.begin(), birthdays.end());
	// Tuple by default sorts by first, then second, then third and so on
